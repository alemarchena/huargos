<?php
    ...
    if($tipo == "consultatodo")
        $sql = $basesql . " where 1 order by apellido asc";
    elseif($tipo == "traenomapeid")
        $sql = "Select idalumno,nombre,apellido,email from $tabla where 1 order by apellido asc";
    elseif($tipo == "consultaxusuario")
        $sql = $basesql . " where usuario ='" . $usuario . "' order by apellido asc";
    else
        $sql = $basesql . " where idalumno =" . $idalumno . " order by apellido asc";
